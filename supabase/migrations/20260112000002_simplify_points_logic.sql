-- Simplify points logic: if points field is cleared (empty), send 0
...
BEGIN
    ...
    -- Simplified: if points provided, use them (even if 0). If NULL, don't update points.
    ...
    -- Update completed_at and status if provided
    ...
    ELSIF p_points_earned IS NOT NULL AND v_trail_participant_record.status = 'completed' THEN
        -- Updating points for already completed trail: update difference
        UPDATE public.user_quests
