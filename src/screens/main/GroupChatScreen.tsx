  ...
  const handleUpdate = async () => {
    ...
    if (headerImageUri) {
      try {
        ...
        Alert.alert('Upload Failed', 'Failed to upload header image.');
        setLoading(false);
        return;
      }
...
export default function GroupChatScreen() {
  ...
  const handleSend = useCallback(async () => {
    ...
    if (selectedFile) {
      setIsUploading(true);
      try {
        const fetchResponse = await fetch(selectedFile.uri);
        const blob = await fetchResponse.blob();
        ...
        const { error: uploadError } = await supabase.storage
          .from('community_groups')
          .upload(filePath, blob, { contentType: selectedFile.type, upsert: false });
        if (uploadError) throw new Error(uploadError.message);
        fileUrl = filePath;
      } catch {
